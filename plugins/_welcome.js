...
const handler = async (m, { conn, args, isAdmin, isOwner }) => {
  if (!isAdmin &&!isOwner) return conn.reply(m.chat, `🪷 𓆩 ***𝗖𝗢𝗧𝗧𝗜 𝗕𝗢𝗧*** 𓆪 🌸\n\n💮 *Solo admins pueden usar este comando*`, m)
  ...
  if (/on/i.test(args[0])) {
    ...
    await conn.reply(m.chat, `🪷 𓆩 ***𝗖𝗢𝗧𝗜 𝗕𝗢𝗧*** 𓆪 🌸\n\n📌 *Uso:* ${m.prefix}bienvenida on/off`, m)
  }
}
...
handler.before = async function (m, { conn, groupMetadata }) {
  ...
  const DEFAULT_IMG = 'https://files.evogb.win/UHUtT3.jpg' // TU FOTO COTTI
  let imgBuffer = null
  ...
  try {
    ...
    // PASO 2: Si falla, usar tu foto cotti default
    try {

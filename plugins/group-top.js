...
function handler(m, { groupMetadata, command, conn, text }) {
    if (!groupMetadata) return m.reply('🐱 *Marie dice: Este comando solo funciona en grupos*')
    if (!text) return m.reply(`🐱 𓆩 ***COTTI BOTS OFICIAL*** 𓆪 🐱

🌸 *Marie dice: Ejemplo de uso:*
.top *Mejores en PVP*
.top *Más activos*`)

    let ps = groupMetadata.participants.map(v => v.id)
    if (ps.length < 10) return m.reply('💖 *Marie dice: Se necesitan mínimo 10 miembros en el grupo*')
